...
// trips
  ...
  trips.forEach(item=>{ item.isCash? cashAmount += +item.billedAmount.toString().replace(/,/g, '') :
                                    nonCashAmount += +item.billedAmount.toString().replace(/,/g, '') })
...
highEarningDriverInfo.totalEarnedAmount = totalEarnedAmount
...
output.cashBilledTotal = +cashAmount.toFixed(2)
output.nonCashBilledTotal = +nonCashAmount.toFixed(2)
